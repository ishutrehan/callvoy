<div class="row">
   ...
   $unblock = DB::table('block_user')
	   ...
	   ->where('user_blocked',$key->user()->id)
	   ->orWhere('user_id',$key->user()->id)
   ...
   /*if( Str::slug( $key->title ) == '' ) {
	...
	$_timeframe = Input::get('timeframe');
	
	$_valid_timeframe = array(
		"week", 
		"month", 
		"year"
	);
...
?>
    		...
    		<div class="thumbnail @if( isset( $shots_users ) ) padding-top-zero @endif">
    			...
    			<div class="caption">
    				...
    				@if( $key->user()->type_account == 2 ) 	
    					...
    					<a href="{{ URL::to('/') }}/{{ '@'.$key->user()->username }}" class="myicon-right text-decoration-none" title="{{ $key->name }}">
    					  {{ e( $key->user()->name ) }} 
    					  </a>
    			...
    			<div class="caption">
    				<h1 class="title-shots">
    					<a title="{{ e($key->title) }}" class="item-link" href="{{$url_shot}}">
    					 {{ e($key->title) }}
    					</a>
    				</h1>
    			</div><!-- /caption -->
    			...
    			<div class="caption">
    				...
    				@if(Auth::check())
    					...
    					@else
    						<span class="pull-left" style="width: 26px; height: 24px;"></span>
    					@endif {{-- Unblock --}}
    				@else
     ...
     @if( $data->getTotal() != $data->count() )
    	   ...
    	   @if( Request::is('search') )
			...
			@elseif( Request::is('/') && !Auth::check() )
			
		<div class="btn-group paginator-style">
			<div class="btn-group paginator-style">
			   <ul class="pager">
				<li><a href="{{URL::to('latest?page=2')}}" rel="next"><i class="glyphicon glyphicon-chevron-right"></i></a></li>
				</ul>
			</div>
		</div>
		 ...
		 @if( isset( $_timeframe ) && in_array( $_timeframe, $_valid_timeframe) && Request::path() != 'latest' )
		 <div class="btn-group paginator-style">
		   <?php echo $data->appends( array( 'timeframe' => $_timeframe ) )->links(); ?> 
		</div>
		 @else
		 <div class="btn-group paginator-style">
		   <?php echo $data->links(); ?> 
