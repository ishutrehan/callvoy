...
<form action="<?php echo $action; ?>" method="post" name="formpaypal">
	...
	<input type="hidden" name="return" 			value="{{URL::to('/')}}/renew/team/success">				
	<input type="hidden" name="notify_url" 		value="{{URL::to('/')}}/ajax/teampaypalipn">
	<input type="hidden" name="custom"          value="team_id={{Auth::user()->id}}&renew=true">			
	<input type="hidden" name="item_name"  		value="<?php echo Lang::get('users.renew_memberships_team'); ?>"> 
	...
	<input type="hidden" name="cancel_return"   value="{{URL::to('/')}}">			
